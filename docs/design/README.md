...
El nombre de cada monitor es `<button class="monitor__open" aria-expanded aria-controls="admin-drawer">` (abre la sala en el cajón; el monitor abierto se marca con `.monitor:has(.monitor__open[aria-expanded="true"])`). El cajón es un diálogo modal (`role="dialog" aria-modal="true"`): mientras está abierto, la franja, Atención, la pared y la agenda quedan `inert`. La estación de una sala `emitter` (Tarea 14a) usa `.station-link` (`__row` con el enlace y "Copiar", `__qr`).
