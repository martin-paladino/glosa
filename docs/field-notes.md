...
> Status on 2026-09-25: all five points below are implemented and merged. (1) The room station, `/station/{room}?key=...`. (2) Remote reload ("F5 remoto"), `POST /api/admin/rooms/{id}/station/reload`. (3) The agenda-driven autopilot (auto/manual per room, `glosa/scheduler.py`). (4) The audience QR flow: `/qr/{room}` (a printable/projectable page with the room's QR code) plus a `qr_only` access mode that hides the room list at `/` and only accepts a room's QR link. (5) The overlay, `/overlay/{room}` -- a transparent, chrome-less page for a vMix browser input or an OBS browser source.
